...
class Application(Frame):
    def __init__(self, master=None):
        ...
        # row 0-1
        Label(master=main_pane, text='ICD file:').grid(column=1, row=0, stick="wn", pady=(5, 0))
        self.icd_entry = Entry(master=main_pane, state='disabled')
        self.icd_entry.grid(column=1, row=1, stick="nsew", padx=(0, 5), pady=(0, 5))
        Button(master=main_pane, relief='flat', takefocus=0, command=self.on_open_icd, image=self.ico_code). \
            grid(column=0, row=0, rowspan=2, stick="nsew", padx=(5, 0), pady=5)
        # row 2-3
        Label(master=main_pane, text='TXT associations file:').grid(column=1, row=2, stick="wn", pady=(5, 0))
        self.txt_entry = Entry(master=main_pane, state='disabled')
        self.txt_entry.grid(column=1, row=3, stick="nsew", padx=(0, 5), pady=(0, 5))
        Button(master=main_pane, relief='flat', takefocus=0, command=self.on_open_txt, image=self.ico_txt) \
            .grid(column=0, row=2, rowspan=2, stick="nsew", padx=(5, 0), pady=5)
        # row 4-5
        Label(master=main_pane, text='IED official name:').grid(column=1, row=4, stick="wn", pady=(5, 0))
        self.ied_entry = Entry(master=main_pane)
        self.ied_entry.grid(column=1, row=5, stick="nsew", padx=(0, 5), pady=(0, 5))
        Button(master=main_pane, relief='flat', takefocus=0, image=self.ico_refresh). \
            grid(column=0, row=4, stick="sew", rowspan=2, padx=(5, 0), pady=(10, 5))
        # row 6
        Button(master=main_pane, relief='flat', command=self.on_mics, image=self.ico_doc). \
            grid(column=0, row=6, stick="nsew", padx=(5, 0), pady=5)
        Label(master=main_pane, text='Run MICSer to create a MISC document.').grid(column=1, row=6, stick="wns")
        # row 7
